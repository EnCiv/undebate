...
export default class ReactCameraRecorder extends React.Component {
    ...
    getCameraStream(constraints={
        ...
        if(this.canNotRecordHere) 
            return Promise.reject(new Error("can not record here"));
        else
    ...
    startRecording(cb) {
        ...
        if(this.startRecorderState.state!=="READY") {
            if(this.startRecorderState.state==="QUEUED") logger.error("Undebate.startRecording queueing but", this.startRecorderState, "already queued");
        ...
        const stopRecording=()=>{
            ...
            if (mediaRecorder && mediaRecorder.state !== 'inactive') {
                ...
                if(recordedBlobs.length)
                    logger.trace('Recorded Blobs: ', recordedBlobs.length && recordedBlobs[0].type, recordedBlobs.length, recordedBlobs.reduce((acc,blob)=>acc+blob.size||0,0));
                else
        ...
        mediaRecorder.onstop = (event) => {  // replace the onstop handler each time, because the saveRecording parameters change
