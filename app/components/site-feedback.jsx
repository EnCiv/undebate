...
const styles = {
  defaultLandscape: {
    ...
    width: IconWidth + IconHeight * 0.2 + 'em',
  ...
  center: {
    // manually determined to place the thumbs up in the center of the comment-alt
    color: '#606060', // mild contrast with background
    left: '.55em',
    top: '.3em',
  },
...
class SiteFeedback extends React.Component {
  elms = {}
  state = { response: null, open: false, width: 0, height: 0 }
  ...
  openClose(e) {
    ...
    this.setState(newState, () => {
      if (willOpen) this.elms.message.value = 'url: ' + window.location.href + '\n\n'
      else this.state.response = '' // clear the response;
    })
  }
  ...
  render() {
    ...
    var feedbackForm = () => {
      // not a function so 'this' is inherited from caller
      return (
        <div className={classes.wrapper} ref={this.openForm}>
          <div className={classes.form}>
            <p>
              Tell us what you like, tell us what you don't like, that's what helps us make this even better. Your
              feedback is appreciated.
            </p>
            <input
              ref={e => {
                e ? (this.elms.subject = e) : null
              }}
            ...
            <div className={classes.buttonRow}>
              <button
                ...
                onClick={() => {
                  this.setState({ open: false, width: 0, height: 0 })
                }}
                name="Cancel"
              >
                Cancel
              </button>
              <button className={classes.button} onClick={this.contactUs.bind(this)} name="Send Feedback">
                Send Feedback
              </button>
            </div>
          </div>
    ...
    return (
      <div
        style={{ bottom: this.state.height * 1.05 }}
        className={cx(
          classes['default' + panelClass],
          this.state.open ? classes['open' + panelClass] : null,
          className
        )}
        title="Comments? Critical Feedback? Complaints? Great! I can't wait to see them."
      >
