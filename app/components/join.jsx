...
class Join extends React.Component {
	...
	onBlurEmail(e){
		...
		if(email && !isEmail(email))
			this.setState({ validationError: 'email address is not valid', info: null });
		else 
			this.setState({validationError: null})
	}

	authenticateSocketIo(cb){  // after joining or loging in the socket needs to reconnect to get the authentication cookie in the header so it can be authenticated. Many api calls require the user to be authenticated
		if(typeof window!=='undefined' && window.socket){
			...
			const onDisconnect=(reason)=>{
				if(reason!=='io client disconnect') logger.info("Join.authenticate unexpected disconnect reason:", reason, "... Continuing")
			...
			.end((err, res) => {
				...
				switch (res.status) {
					...
					case 200:
						...
						if (this.props.onChange) this.authenticateSocketIo(() => this.props.onChange({ userId: JSON.parse(res.text).id }));
	...
	onChangeActive() {
		...
		if (this.state.loginActive && (!email || !isEmail(email) || !password || confirm)) this.setState({ loginActive: false });
		if (!this.state.joinActive && email && isEmail(email) && password && confirm && password == confirm && agree) this.setState({ joinActive: true });
		if (this.state.joinActive && (!email || !isEmail(email) || !password || !confirm || password != confirm || !agree)) this.setState({ joinActive: false });
	}
