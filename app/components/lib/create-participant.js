...
export default function createParticipant(props, human, userId, name, progressFunc){
    ...
    function upload(blob,seat,round) {
        var file_name = userId + '-' + round + '-' + seat + new Date().toISOString().replace(/[^A-Z0-9]/ig, "") + '.mp4'; // mp4 was put here to get around something with Apple - check in future 
        ...
        const responseUrl=(url)=>{
            ...
            if(url){
                ...
                if(seat==='speaking') {
                    ...
                    participant.listening=url;

            }else {
                logger.error("upload video failed", file_name)
            }
            if(participant.speaking.length===human.speakingBlobs.length && participant.listening){ // have all of the pieces been uploaded
                ...
                if(props.bp_info) {// don't cause the property to exist in the Iota if there is none. 
