...
class RASPUndebate extends React.Component {
  ...
  constructor(props) {
    ...
    if (typeof window !== 'undefined') {
      if (!supportsVideoType('webm')) {
        ...
        else this.canNotRecordOnSafari = true
      }
      if (props.participants.human) {
        if (typeof MediaRecorder === 'undefined') this.canNotRecordOnSafari = true
      }
    }
        ...
        this.participants[participant] = {
          ...
          placeholderUrl:
            ...
            this.props.participants[participant].listening
              .split('/')
              .reduce((acc, part) => acc + (acc ? '/' : '') + part + (part === 'upload' ? '/so_0' : ''), '')
              .split('.')
              .reduce(
                (acc, part) => (part === 'webm' || part === 'mp4' ? acc + '.png' : acc + (acc ? '.' : '') + part),
                ''
              ),
          youtube,
        }
  ...
  startRecording(speaking, round) {
    ...
    if (this.startRecorderState.state !== 'READY') {
      if (this.startRecorderState.state === 'QUEUED')
        logger.error(
          'Undebate.startRecording queueing',
          { speaking, round },
          'but',
          this.startRecorderState,
          'already queued'
        )
  ...
  stopRecording() {
    ...
    if (this.mediaRecorder && this.mediaRecorder.state !== 'inactive') {
      ...
      if (this.recordedBlobs.length)
        logger.trace(
          'Recorded Blobs: ',
          this.recordedBlobs.length && this.recordedBlobs[0].type,
          this.recordedBlobs.length,
          this.recordedBlobs.reduce((acc, blob) => acc + blob.size || 0, 0)
        )
  ...
  saveRecordingToParticipants(speaking, round) {
    ...
    if (!this.recordedBlobs.length)
      return logger.error('saveRecordingToParticipants found no blobs', this.mediaRecorder.state)
    logger.trace(
      'save Recorded Blobs: ',
      this.recordedBlobs.length,
      this.recordedBlobs.length && this.recordedBlobs[0].type,
  ...
  prevSpeaker() {
    ...
    if (this.numParticipants === 1) {
      ...
      if (seatOffset === 0) {
        //we were listening to the moderator
        if (round > 0) {
          round--
  ...
  upload(blob, seat, round, userId) {
    var name = userId + '-' + round + '-' + seat + new Date().toISOString().replace(/[^A-Z0-9]/gi, '') + '.mp4' // mp4 was put here to get around something with Apple - check in future
    ...
    const responseUrl = url => {
      if (url) {
        ...
        } else this.participant.listening = url
      } else {
        logger.error('upload video failed', name)
      }
      if (
        this.participant.speaking.length === this.props.participants.audience1.speaking.length &&
        this.participant.listening
      ) {
    ...
    const hangupButton = () =>
      ...
      this.participants.human && (
        <div style={{ height: '5.5rem' }}>
          <button className={classes['hangUpButton']} onClick={this.hangup} key="hangup">
            Hang Up
          </button>
        </div>
      )
