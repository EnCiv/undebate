...
const styles = {
    ...
    'agenda': {
        position: 'absolute',
        top: 'calc(50vw * 0.5625 + 2.5vw)',
        left: 'calc(2.5vw + 20vw + 2.5vw + 25vw - 7.5vw)',
...
const seating = ['speaking', 'nextUp', 'seat2']
const seatToName={
    speaking: "Speaking",
    nextUp: "Next Up",
    seat2: "Seat #2",
...
class RASPUndebateLine extends React.Component {
    ...
    downloadRecording() {
        ...
        let oldSeatOffset = seatOffset + 1 % Object.keys(this.props.participants).length;
        let seat=this.seat(Object.keys(this.props.participants).indexOf('human'), oldSeatOffset);
    ...
    async playObjectURL(part, objectURL, speaking) {
        ...
        try {
            if(element.src) {
                //await element.pause();
                // doing this to prevent the screen from flickering
            ...
            if(typeof err==='object'){
                if (err.name === "NotAllowedError") {
                    ...
                    logger.error("AskWebRTC.startPlayback caught error", err);
                }
            }else{
                this.debugOverlay(`play caught unknown error: ${err.toString()}`);
                logger.error("UndebateLine caught unknown error", err);
            }
        }
    }
    ...
    render() {
                ...
                <div className={cx(classes['agenda'], finishUp && classes['finishUp'])} key={'agenda' + this.state.round}>
                    <div className={classes['innerAgenda']}>
                        ...
                        <ol className={classes['agendaItem']}>
                            {this.props.participants.moderator.agenda[this.state.round] && this.props.participants.moderator.agenda[this.state.round].map((item, i) => <li key={item + i}>{item}</li>)}
                        </ol>
                    </div>
                </div>
            )
        ...
        return (
            <section id="syn-ask-webrtc" key='began' style={{position: 'relative', left: this.state.left, width: '100vw'}} ref={this.fixupLeft}>
                ...
                <div className={classes['outerBox']}>
                    {Object.keys(this.props.participants).map(videoBox)}
                    {agenda()}
                </div>
                {beginOverlay()}
                {permissionOverlay()}
...
export default injectSheet(styles)(UndebateLine);


/** leavig these buttons out for now
 * 
 * 
 * 
