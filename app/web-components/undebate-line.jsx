...
class RASPUndebateLine extends React.Component {
  ...
  componentDidMount() {
    ...
    let i
    for (i = 0; i < this.props.participants.moderator.speaking.length; i++) {
  ...
  rotateOrder() {
    ...
    if (seatOffset < 0) {
      if (this.props.participants.moderator.speaking[round + 1]) seatOffset = seating.length - 1
        ...
        <div className={cx(classes['agenda'], finishUp && classes['finishUp'])} key={'agenda' + this.state.round}>
          <div className={classes['innerAgenda']}>
            ...
            <ol className={classes['agendaItem']}>
              {this.props.participants.moderator.agenda[this.state.round] &&
                this.props.participants.moderator.agenda[this.state.round].map((item, i) => (
