...
//            "bigLine": "",
        ...
        "participants": {
            "moderator": {
                ...
                "listening": "https://res.cloudinary.com/hf6mryjpf/video/upload/v1573175176/5d5b73c01e3b194174cd9b92-5-speaking20191108T010614689Z.webm",
                "agenda": [
                    [
                        "Introductions",
                        "1- Name",
...
function updateOrCreatePair(raceObj){
    ...
    return new Promise(async (ok,ko)=>{
        ...
        if(viewers.length==0){ // create the new race
            ...
            if(recorders.length===0){ // it didn't exist
                createNewRecorder(viewerObj);
                return
            } else {
...
if(args.src && args.db){
    ...
    .catch(err=>{console.error("csv caught error", err)})
} else {
