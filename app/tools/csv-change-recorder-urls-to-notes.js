    ...
    .then(async csvRowObjList => {
      ...
      for await (const csvRowObj of csvRowObjList) {
        ...
        try {
          ...
          console.info('change', myURL.pathname, '_id:', recorder._id.toString())
          await db.collection('iotas').updateOne(
            { path: myURL.pathname },
            {
              $set: {
                webComponent: {
                  webComponent: 'Note',
                  templateId: 'pres-not-available',
                  title: 'Not Available',
                  content:
                    "We're sorry, this link has been taken down. Please contact the referrer for more information.",
                },
              },
              $unset: { component: '' },
            }
